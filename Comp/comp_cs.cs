...
public class comp_cs : MonoBehaviour
{
	...
	void FixedUpdate ()
	{
		...
		else //Human
		{
			...
			else if (Input.acceleration.z >= 0 && Input.acceleration.z <= 1)
				anim.SetInteger ("State", -1);
//			else if (Input.acceleration.x >= -1 && Input.acceleration.x <= 0)
...
//				anim.SetInteger ("State", -10);
			...
			//Turn
			if(Input.GetKey(KeyCode.D)&& velZ!=0 || Input.acceleration.x >= 0 && Input.acceleration.x <= 1) ang = Mathf.Lerp(ang,1.0f,0.01f);
			else if(Input.GetKey(KeyCode.A)&& velZ!=0 || Input.acceleration.x >= -1 && Input.acceleration.x <= 0)ang = Mathf.Lerp(ang,-1.0f,0.01f);
